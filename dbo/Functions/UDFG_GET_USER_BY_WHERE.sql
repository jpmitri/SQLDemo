CREATE FUNCTION [DBO].[UDFG_GET_USER_BY_WHERE]
(
@P__OWNER_ID INT,
...
SELECT
[T].*
FROM  [DBO].[TBL_USER] [T]  WITH (NOLOCK)
WHERE 
(COALESCE([USERNAME],'') LIKE @V__USERNAME) AND 
(COALESCE([PASSWORD],'') LIKE @V__PASSWORD) AND 
(COALESCE([USER_TYPE_CODE],'') LIKE @V__USER_TYPE_CODE) 
